...
def generate_minhash_Array(users,num_hashes, num_users):
    ...
    for user in users:
        for i in range(1, num_hashes+1):
            current_hash_code = ((i*user)+(3*i*13)) % num_users
...
def generateRandomHashCoefficients(num_hashes):
    random_coeffs = []

    while num_hashes > 0:
        random_a = random.randint(0,99999)
        random_b = random.randint(0,99999)

        random_p = random.randint(0,99999)

        while not isPrime(random_p):
            random_p = random.randint(0,99999)

        random_coeff = tuple(random_a, random_b, random_p)
    if random_coeff not in random_coeffs:
        random_coeffs.append()
        num_hashes -= 1
...
business_user_map = finalRdd\
    .map(lambda entry: (entry[1], hash(entry[0])))\
    .groupByKey()\
    .sortByKey()\
...
# random_ab= generateRandomHashCoefficients(num_hashes)
